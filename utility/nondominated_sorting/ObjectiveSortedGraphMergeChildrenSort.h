...
namespace NDS {
	...
	public:
		void ObjectiveSortedGraphMergeChildrenSort(const vector<vector<double>> & data,vector<int> & dataRank) {
			int nodeNum(data.size());
			if (nodeNum == 0)return;
			dataRank.resize(data.size());
			int objNum = data.front().size();
			vector<vector<int>> nodeObjectiveHead(nodeNum);
			for (auto & it : nodeObjectiveHead) {
				it.resize(objNum);
			}
			vector<vector<L_Node>> nodeObjectiveIndex(nodeNum);
			...
			L_Node LNend;
			vector<vector<L_Node>> totalObejctiveLink(objNum);
			vector<int> nodeSum(nodeNum,0);
			vector<int> nodeFromObjective(nodeNum, 0);
			
			// initialize the objectiveGraph   time: O(m*n*lg(n)))

			{
				vector<int> nodeObjBeforeCur(nodeNum,nodeNum + 1);
				// temp memory, first: data,second : index
				vector<pair<double, int>> curObjLink(nodeNum);
				vector<int> preSum(nodeNum);
			...
			vector<L_Node> nodeCurrentIndex(nodeNum);
			for (int i(0); i < nodeNum; ++i) {
				nodeCurrentIndex[i].m_id=i;
			}
			L_Node visitingNodeHead(-1, false, nullptr, &LNend);
			vector<L_Node> nodeChildren(nodeNum);
			for (auto & it : nodeChildren) {
				it.m_after = &LNend;
			}
			vector<pair<int, int>> nodeObjectiveSum(nodeNum);
		...
		// recommemded
		void RankObjectiveLinkSort(const vector<vector<double>> & data, vector<int> & dataRank) {
			int nodeNum(data.size());
			if (nodeNum == 0)return;
			dataRank.assign(nodeNum,-1);
			int objNum(data.front().size());
			vector<int> objectiveLastRank(objNum, 0);
			L_Node LNend;
			vector<vector<L_Node>> rankObjeciveHead(objNum);
			...
			vector<vector<L_Node>> nodeObjectiveNode(objNum);
			...
			vector<vector<pair<double, int>>> objectiveSortedNode(objNum);
			vector<vector<int>> objectiveSortedSameNode(objNum);// remember the index of node with the same objective value
			vector<int> nodeObjectiveRankSum(nodeNum, 0);
			int curRank(0);
			...
			int nodeIndex(0);
			int objIndex(0);
			vector<int> parents(nodeNum, nodeNum);
			vector<int> objCompareIndex(nodeNum+1,0);
			int iter(0), tmp(0);
			...
			while (totalSolvedNode < nodeNum) {
				for (objIndex = 0; objIndex < objNum; ++objIndex) {
					...
					if (dataRank[tmp]==-1) {
						...
						while (iter >= 0 && dataRank[tmp] == -1) {
							...
							while (iterPointer->m_after->m_isUsed) {
								...
								objCompareIndex[parents[iterPointer->m_id]] = max(objCompareIndex[parents[iterPointer->m_id]], objNum*totalSolvedNode+ objCompareIndex[iterPointer->m_id]);
							}
							--iter;
						}
					...
					objectiveLastRank[objIndex] = max(objectiveLastRank[objIndex], dataRank[tmp]);
	...
	private:
		...
		bool compareDominated(const vector<double> & father,const vector<double> & son,int objSize) {
		...
		int compareDominated(const vector<double> & father, const vector<double> & son, int& fromIndex, int objSize) {
			int ret = 0;
